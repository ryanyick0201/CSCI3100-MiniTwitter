...
function findRoom(userIdPair) {
    ...
    if (userIdPair[0] < userIdPair[1])    { sortedUserIdPair = userIdPair}
    else {sortedUserIdPair = [userIdPair[1], userIdPair[0]]}

    return rooms.findIndex(o => (o[0] == sortedUserIdPair[0] && o[1] == sortedUserIdPair[1])).toString();
}
...
function addRoom(userIdPair){
    ...
    if (userIdPair[0] < userIdPair[1])    {const sortedUserIdPair = userIdPair}
    else {const sortedUserIdPair = [userIdPair[1], userIdPair[0]]}

    console.log('now room is', rooms);
    return (rooms.push(usernameArr) - 1).toString();
}
