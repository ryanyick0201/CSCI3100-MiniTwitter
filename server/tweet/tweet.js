...
async function searchOthersTweetByMultiple(myUsername, lookForUsername, category){
    if (!myUsername)
        throw (`{"message": "myUsername missing."}`);   
    ...
    myUserId = JSON.parse(await searchUserByUsername(myUsername, true))['result'][0]['userId'];
    lookForUserId = JSON.parse(await searchUserByUsername(lookForUsername, true))['result'][0]['userId'];
    ...
    console.log(tweets);
    return `{"message": "Retrieve succeeded", "result": ${JSON.stringify(tweets)}}`
}
