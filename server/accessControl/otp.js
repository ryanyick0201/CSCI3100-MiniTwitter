...
async function setOTP(username) {
  try {
    ...
    x = await query(`UPDATE User Set otp = ? WHERE username = ?`, [
      otp,
      username,
    ]);
...
async function verifyOTP(username, otp) {
  try {
    ...
    if (!x[0].otp) throw `{"message": "Verify otp failed. Otp has expired."}`;

    x = await query(`SELECT * FROM User WHERE username = ? AND otp = ?`, [
      username,
      otp,
    ]);
    if (x.length === 1) {
      await query(
        `UPDATE User SET hasVerified = true where username = "${username}"`
      );
...
async function deleteOTP(username) {
  try {
    x = await query(`UPDATE User Set otp = NULL WHERE username = ?`, [
      username,
    ]);
